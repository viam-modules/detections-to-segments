...
- **Native point cloud (preferred):** if the camera advertises `SupportsPCD` and exposes intrinsic parameters, the module calls `NextPointCloud` and keeps the points whose projection onto the image plane (using both intrinsics and extrinsics) falls inside the detection's bounding box. This preserves the fidelity of cameras that produce their own registered/fused point clouds. Per-point color is carried through when the native cloud has it; segments will be uncolored otherwise.
