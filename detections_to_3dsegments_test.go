...
func TestMinPointsFiltering(t *testing.T) {
	...
	seg, err := DetectionSegmenter(objectdetection.Detector(detector), 1, 1.0, 0.5, 0, 100)
	test.That(t, err, test.ShouldBeNil)
...
func TestPinholeDeprojection(t *testing.T) {
	...
	seg, err := DetectionSegmenter(objectdetection.Detector(detector), 1, 10.0, 0.5, 0, 0)
	test.That(t, err, test.ShouldBeNil)
